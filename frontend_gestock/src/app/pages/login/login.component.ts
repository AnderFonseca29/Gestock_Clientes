...
export class LoginComponent {
  ...
  // Variables vinculadas con [(ngModel)]
  email: string = '';
  password: string = '';
  ...
  login(): void {
    ...
    // Redirección hacia la ruta del panel bajo el layout (/app/panel)
    this.router.navigate(['/app/panel']).then((navegadoConExito) => {
      if (!navegadoConExito) {
        console.warn('La navegación a /app/panel fue bloqueada. Revisa tus rutas o AuthGuard.');
      }
    }).catch((error) => {
